...
class SplitImageAssetsPackageTests(SplitImageAssetsTestBase):
    ...
    def test_prepare_plan_manifest_clear_selection_only(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(clear_result.returncode, 0, clear_result.stderr)
            plan_manifest = json.loads((output / "plan_manifest.json").read_text(encoding="utf-8"))
            self.assertEqual(plan_manifest["scope_selection"]["candidate_families"], ["blueprint-modules", "paper-scraps"])
            self.assertEqual(plan_manifest["scope_selection"]["selected_family"], "")
            self.assertEqual(plan_manifest["scope_selection"]["selection_source"], "unresolved")
            self.assertEqual(plan_manifest["scope_selection"]["selection_evidence_ref"], "")
            self.assertEqual(plan_manifest["scope_selection"]["selection_notes"], "")
    def test_prepare_plan_manifest_preserves_selection_when_adding_note(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(plan_manifest["scope_selection"]["selected_family"], "blueprint-modules")
            self.assertEqual(plan_manifest["scope_selection"]["selection_source"], "explicit-user-confirmed")
            self.assertEqual(plan_manifest["scope_selection"]["selection_evidence_ref"], "chat:family-confirmed")
            self.assertEqual(plan_manifest["scope_selection"]["selection_notes"], "added note only")
    def test_prepare_plan_manifest_clear_then_replace_selection(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(update_result.returncode, 0, update_result.stderr)
            plan_manifest = json.loads((output / "plan_manifest.json").read_text(encoding="utf-8"))
            self.assertEqual(plan_manifest["scope_selection"]["selected_family"], "paper-scraps")
            self.assertEqual(plan_manifest["scope_selection"]["selection_source"], "explicit-user-confirmed")
    ...
    def test_describe_candidate_work_items_reports_provider_ids_for_staged_candidates(self):
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = pathlib.Path(tmp)
            source = tmp_path / "source.png"
            Image.new("RGBA", (4, 3), (10, 20, 30, 255)).save(source)
            ...
            self.assertEqual(result.returncode, 0, result.stderr)
            entry = json.loads(result.stdout)["objects"][0]
            self.assertEqual(entry["candidate_provider_ids"], ["codex-controlled-generation"])
            self.assertTrue(
                entry["candidate_provider_stage_manifest_paths"][0].endswith("candidate-a_provider_stage.json")
            )
    def test_describe_candidate_work_items_flags_mixed_provider_candidate_pool(self):
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = pathlib.Path(tmp)
            source = tmp_path / "source.png"
            Image.new("RGBA", (4, 3), (10, 20, 30, 255)).save(source)
    ...
    def test_describe_candidate_work_items_recommends_promote_single_candidate(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(entry["recommended_task"]["task_type"], "candidate-lifecycle")
            self.assertEqual(entry["recommended_task"]["task_phase"], "candidate-promotion")
            ...
            self.assertEqual(
                entry["recommended_command_variants"][0]["writes_fields"],
                ["confirmation.candidate_promotion", "current_asset_revision", "repair_history"],
            )
            self.assertEqual(entry["recommended_command_variants"][0]["next_action_if_success"], "no-candidate-work-required")
    ...
    def test_describe_candidate_work_items_awaits_candidate_selection_after_compare(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(entry["recommended_task"]["task_type"], "candidate-lifecycle")
            self.assertEqual(entry["recommended_task"]["task_phase"], "candidate-selection")
            ...
            self.assertEqual(
                entry["recommended_command_variants"][0]["writes_fields"],
                ["selected_candidate_id", "decision_log"],
            )
            self.assertEqual(entry["recommended_command_variants"][0]["next_action_if_success"], "record-candidate-promotion-approval")
    def test_describe_candidate_work_items_recommends_selection_without_candidate_id_for_single_candidate_compare(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                entry["recommended_command_variants"][1]["writes_fields"],
                [
                    "selected_candidate_id",
                    "decision_log",
    ...
    def test_record_candidate_selection_resolves_provider_specific_comparison(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(payload["comparison_id"], "cmp-b")
            self.assertEqual(payload["candidate_id"], "candidate-b")
    def test_apply_candidate_selection_decision_records_selection_only(self):
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = pathlib.Path(tmp)
            source = tmp_path / "source.png"
            Image.new("RGBA", (4, 3), (10, 20, 30, 255)).save(source)
            ...
            result = subprocess.run(
                [
                    sys.executable,
                    str(ROOT / "scripts" / "apply_candidate_selection_decision.py"),
                    str(output),
                    "--object-id",
                    "main_object",
                    "--comparison-id",
            ...
            self.assertEqual(result.returncode, 0, result.stderr)
            payload = json.loads(result.stdout)
            self.assertEqual(payload["selection"]["candidate_id"], "candidate-a")
            self.assertIsNone(payload["promotion_decision"])
    def test_apply_candidate_selection_decision_selects_and_promotes(self):
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = pathlib.Path(tmp)
            source = tmp_path / "source.png"
            Image.new("RGBA", (6, 6), (20, 20, 20, 255)).save(source)
            ...
            payload = json.loads(result.stdout)
            self.assertEqual(payload["selection"]["candidate_id"], "candidate-a")
            self.assertEqual(payload["promotion_decision"]["decision_answer"], "yes")
            metadata = json.loads((output / "metadata.json").read_text(encoding="utf-8"))
            self.assertEqual(metadata["objects"][0]["selected_candidate_id"], "candidate-a")
            self.assertEqual(metadata["confirmation"]["candidate_promotion"]["status"], "confirmed")
