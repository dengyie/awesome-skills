...
8. Extract, reconstruct, generate, or import assets according to the planned object routes and provider results.
   ...
   - when staged repair candidates exist, `describe_candidate_work_items.py` should write `_staging/repair_candidates/candidate_work_items.json` so compare versus promotion next steps are explicit; `recommended_command` stays as the compatibility/default next step, `recommended_command_variants[]` should surface meaningful lifecycle branches when they exist, and `recommended_task` should group those branches into one machine-readable lifecycle task when applicable
   ...
   - when compare evidence exists but no selected candidate has been recorded yet, use `record_candidate_selection.py` to record `selected_candidate_id` and `selection_reason` before approval or promotion
   - `apply_candidate_selection_decision.py` may wrap that same selection step and continue directly into a yes/no promotion decision when the user already knows whether promotion should proceed
