...
8. Extract, reconstruct, generate, or import assets according to the planned object routes and provider results.
   ...
   - when staged repair candidates exist, `describe_candidate_work_items.py` should write `_staging/repair_candidates/candidate_work_items.json` so compare versus promotion next steps are explicit; `recommended_command` stays as the compatibility/default next step and `recommended_command_variants[]` should surface meaningful lifecycle branches when they exist, including machine-readable branch metadata
