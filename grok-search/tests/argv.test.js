...
await withServer(
  ...
  async (_server, port) => {
    ...
    assert.equal(Object.hasOwn(output.content, "full_path"), false);

    const fullPathFetch = await runNode([
      "scripts/fetch.js",
      "--provider",
      "direct",
      "--full-path",
    ...
    assert.equal(fullPathFetch.code, 0);
    output = parseJson(fullPathFetch.stdout);
