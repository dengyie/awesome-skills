...
默认的确定性主入口是 `collect-review-context.py`。其他小脚本更适合做子集自动化、调试或集成胶水。

## `.skill-source-dir` 是什么？

它是安装到本地 skill 副本中的一个小型元数据文件。

它的作用只有一个：记录这次安装来自哪个 repo checkout，这样 `update-local-skill.sh` 之后才能安全地回到那个来源做刷新。

把它视为本地安装契约的一部分，而不是可以随手清理掉的缓存文件。
