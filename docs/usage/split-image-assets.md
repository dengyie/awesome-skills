...
When staged candidates exist, run `describe_candidate_work_items.py` to write `_staging/repair_candidates/candidate_work_items.json`. It will tell you whether candidate stage is still empty, whether compare evidence is needed, whether a candidate selection is still pending after compare, whether a candidate is ready for promotion, or whether candidate work is already complete. Keep reading both `recommended_command` and `recommended_command_variants[]`: the first stays as a compatibility/default suggestion, while the variants make explicit when selection or promotion has safe `skip`, `yes`, or `no` branches. Variants now also expose machine-readable `phase`, `intent`, branch flag/value, `recommended`, and `requires_fields` metadata so a downstream agent can decide which path still needs user input.
