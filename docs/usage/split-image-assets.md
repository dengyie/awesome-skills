...
When staged candidates exist, run `describe_candidate_work_items.py` to write `_staging/repair_candidates/candidate_work_items.json`. It will tell you whether candidate stage is still empty, whether compare evidence is needed, whether a candidate selection is still pending after compare, whether a candidate is ready for promotion, or whether candidate work is already complete. Keep reading three layers together: `recommended_command` for compatibility, `recommended_command_variants[]` for explicit branch options, and `recommended_task` when the state has a grouped lifecycle task. Variants now also expose machine-readable `phase`, `intent`, branch flag/value, `recommended`, `requires_fields`, `writes_fields`, and `next_action_if_success` metadata so a downstream agent can decide which path still needs user input and what state transition it causes.
